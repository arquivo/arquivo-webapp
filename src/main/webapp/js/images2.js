...
function insertImageViewer(imageObj, position) {
  return '' +
    ...
    (imageObj.imgAlt !== "" ? ' <ion-item id="image-viewer-img-src' + position + '" lines="none"><h5><em>' + Content.images.details.alt + ' </em> ' + imageObj.imgAlt + '</h5></ion-item>' : '') +
    (imageObj.imgCaption !== "" ? ' <ion-item id="image-viewer-img-src' + position + '" lines="none"><h5><em>' + Content.images.details.caption + ' </em> ' + imageObj.imgCaption + '</h5></ion-item>' : '') +
...
function viewDetails(position) {
  ...
  if ($('#detailsCard' + position).length == 0) {
    var detailsCard = '' +
      ...
      '<ion-card-content>' +
      '<ion-list>';
    Object.keys(imageObj).forEach(function (key) {
      var value = imageObj[key];
      if (Array.isArray(value))
        value = "\"" + value.join("\", \"") + "\""
      else if (typeof value === "string")
        value = "\"" + value + "\""
      detailsCard += '<ion-item lines="none" ><h5><em>' + key + ':</em> ' + value + '</h5></ion-item>';
    });
    detailsCard += '</ion-list>' +
      '</ion-card-content>' +
      '</ion-card>' +
...
function searchImagesJS(dateStartWithSlashes, dateEndWithSlashes, safeSearchOption, startIndex) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      imageObjs = responseJson.responseItems
      ...
      if (totalResults === 0 || startIndex >= totalResults) {
        ...
        for (var i = 0; i < currentResults; i++) {
          ...
          if (!currentDocument.imgCaption) {
            imageObj.imgCaption = '';
          } else {
            imageObj.imgCaption = currentDocument.imgCaption[0];
          }
          imageObj.imgCaptionFull = imageObj.imgCaption;
          if (imageObj.imgCaption.length > 200) {
            imageObj.imgCaption = imageObj.imgCaption.substring(0, 197) + "...";
          }
          ...
          if (imageObj.imgAlt.length > 40) {
            imageObj.imgAlt = imageObj.imgAlt.substring(0, 37) + "...";
          }
          ...
          if (imageObj.title.length > 40) {
            imageObj.title = imageObj.title.substring(0, 37) + "...";
          }
